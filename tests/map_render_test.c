...
int main(void)
{
    ...
    sh_map_render_default(&a);assert(!a.custom&&a.value[0]==60000&&a.value[1]==0&&sh_map_render_valid(&a));
    assert(sh_map_render_encode(&a,text,sizeof text)&&sh_map_render_decode(text,&b));
    assert(!memcmp(&a,&b,sizeof a));
    assert(sh_map_render_decode("1;60000;0;1500;6500;.35;.4;.45",&b)&&b.custom);
    a.custom=1;
