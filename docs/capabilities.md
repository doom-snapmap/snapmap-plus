...
| AI Navigation on a volume | Enable **AI Navigation** and **Block Demons** on Blocking Boxes. Green outlines show the generated navigation preview; `navmesh.preview` controls its display. The [player guide](../site/snapmap-plus-guide.md) explains authoring. Backend marker, geometry and bake rules are documented in their source files. |
