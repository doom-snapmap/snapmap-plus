...
[architecture](architecture.md#the-30-hz-manual-think-loop) for command execution and declaration-commit threading.
...
| AI Navigation on a volume | Enable **AI Navigation** and **Block Demons** on Blocking Boxes. AI Navigation uses the existing `flags.noFlood` field; old `affectsNavmesh` marks migrate on load and the native flag is cleared. See [marker compatibility](navigation-markers.md). The bake constructs exposed, slope-eligible surfaces from their oriented solid geometry, preserves support unions and floating decks, and connects areas using the class's walking and traversal limits. Complete editor snapshots refresh roughly once a second and once more before Play, including moves, rotations, copies and deletions. Each placed module has its own bake, including repeated Grid Rooms. Green outlines and grid lines display the validated monster48 areas; `navmesh.preview` controls the display. See [Blocking Box navigation](navigation.md) for clearance, preview meaning, module ownership and current boundaries. |
