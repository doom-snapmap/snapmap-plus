...
| Navigation tab | Marks which of your Blocking Boxes demons are allowed to walk on. SnapMap's AI routes on a navmesh baked into each shipped module, so anything you build inside one is invisible to demons — they walk the module floor and nothing else. Ticking a box here sets `affectsNavmesh` on it, and at map load Snapmap+ extends that module's navigation with a walkable area on the box's **top** surface, spliced into the navigation tree so the engine can actually find it. Surfaces within 18 units of each other (the engine's own `maxStepHeight`) are joined so demons walk between them; a surface further up than that is reachable only by spawning demons on it, until baked climb animations land. `affectsNavmesh` is a vanilla field the shipped editor already declares a path for and nothing in the game consumes, so a map full of marked boxes still loads and plays for people without Snapmap+ — they simply get the stock navigation. `sh_navmesh` reports what was marked and what was baked. |
