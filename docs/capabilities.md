...
| Rawmap file menu | **File ▸ Load Rawmap... / Save Rawmap As...** choose either file by hand, from any folder and under any name, through a normal Windows file picker; the menu also shows both staged paths, keeps saves on a chosen file, toggles the swap arm, and restores the defaults. A picked load file is checked before it is accepted (readable, non-empty, at most 64 MB, and starting like JSON), so choosing a compressed `map.decl` or the wrong file is refused at the click with a reason rather than substituting unusable bytes into a later map load. **Loading stages a file; it does not open a map.** The staged rawmap is substituted into the *next* map the engine loads, so open or reload a map to see it — nothing in the UI can drive an editor load on demand. |
