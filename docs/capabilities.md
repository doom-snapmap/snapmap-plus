...
`packages.embed_in_saved_maps` (on by default), and `navmesh.enabled` and
`navmesh.embed_in_saved_maps` (both on by default). Show Hidden and the exclusive Entity
...
| Rawmap save | When rawmaps are on, mirror each saved map's JSON to the staged save file — `%LOCALAPPDATA%\snapmap-plus\rawmap.json` unless **File ▸ Save Rawmap As...** points it somewhere else. Off, your save is untouched and that file is left alone — the same switch governs both directions. |
| Rawmap file menu | **File ▸ Load Rawmap... / Save Rawmap As...** choose either file by hand, from any folder and under any name, through a normal Windows file picker; the menu also shows both staged paths, toggles the swap arm, and restores the defaults. A picked load file is checked before it is accepted (readable, non-empty, at most 64 MB, and starting like JSON), so choosing a compressed `map.decl` or the wrong file is refused at the click with a reason rather than substituting unusable bytes into a later map load. **Loading stages a file; it does not open a map.** The staged rawmap is substituted into the *next* map the engine loads, so open or reload a map to see it — nothing in the UI can drive an editor load on demand. |
...
| AI Navigation on a volume | Tick **AI Navigation** on a Blocking Box and demons can walk on its top surface. The property is the game's own `affectsNavmesh` field on `snapmaps/volume/blocking`, shown in DOOM's Object Properties panel with its own description, so a map that uses it is an ordinary map: nothing is added to it, and a player without Snapmap+ loads and plays it with the module's shipped navigation. Snapmap+ builds the navigation while the map loads, adding one walkable area per marked volume plus climb links up to it, for each of the three demon sizes the engine asks for. Climb heights come from the game's own traversal table, so a demon that cannot climb that high simply does not; a platform nothing can reach is reported as an island by `sh_navmesh` rather than silently doing nothing. Marks are read from the map on load and re-read from the live editor, so a volume ticked and played in the same session takes effect without saving first. Regions may be marked in only one copy of a module the map places more than once -- every copy is built from the same navigation, so they cannot be given different areas, and a map that breaks the rule keeps its shipped navmesh for that module and says so. `navmesh.enabled` turns it off. |
