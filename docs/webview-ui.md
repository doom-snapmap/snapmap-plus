...
- **Create from selection: two crash bugs -- FIXED (2026-07-06).** `serialize_selection` (+0xb0) used to
  hard-crash DOOM outright (an undersized `PREFAB_TEMP_SIZE` stack buffer), then, once that was fixed, crash
  ...
  Fixing the second bug also surfaced a genuine, non-bug engine requirement: Create-from-selection needs the
  editor to be hovering a selected entity (see [`fidelity.md`](fidelity.md)). The webview UI now checks the
