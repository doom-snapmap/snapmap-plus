 ...
 * the full masked pattern at each anchor hit. The only structural difference is that the Python
...
const sig_entry BACKEND_ENGINE_SIGNATURES[] = {
    ...
    { "ConnectOutputCreator", /* FUN_140cdbb40 -- the editor Add-Logic OUTPUT-list creator. linkany.c
                               ...
                               * build): tools/re/run.ps1 _ct_sigcheck.py / decompile FUN_140cdbb40. */
     ...
     * These two engine lookups ARE sig-able + unique (sig_extract resolve==known_rva). */
     ...
     * recipe (file-image, capstone): tools/re/_sig_author.py dump <rva> <len> | scan "<pattern>" against
     ...
     * volatile). re-derive on a build bump: re-run _sig_author.py scan, confirm 1 hit. --- */
